...
struct table{
  ...
  std::mutex m;
  auto insert(std::pair<std::string, std::list<std::string>> pair){
...
struct queue{
  ...
  auto pop_front(){                       //Now returns front and pops in one as front could be popped between calls
    std::unique_lock<std::mutex> lock(m);
    auto front= workQ.front();
    workQ.pop_front();
    return front;
  }
  void push_back(std::string stringToAdd){
    std::unique_lock<std::mutex> lock(m);
    workQ.push_back(stringToAdd);
  }
};
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = &theTable.theTable[name];
...
void workOnFiles(){
  while ( workQ.size() > 0 ) {
      ...
      process(filename.c_str(), &theTable.theTable[filename]);
    }
}
...
int main(int argc, char *argv[]) {
  ...
  char *charCrawlerThreads =getenv("CRAWLER_THREADS");
  int crawlerThreads=8;
  if(charCrawlerThreads!=NULL){
    crawlerThreads=atoi(charCrawlerThreads);
  };
  ...
  //create threads and run until workQ is done
  std::vector<std::thread> threads;
  for(int i=0; i< crawlerThreads;i++){
      threads.push_back(std::thread(workOnFiles));
  }
  for(int i=0; i<threads.size();i++){
    threads.at(i).join();
  }

  //check for when queue empty then run next
