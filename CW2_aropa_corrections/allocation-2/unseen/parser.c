...
void reset_statement() {
	...
	switch (stmt.type) {
		...
		mem_free(stmt.sql.regist.automaton);
		mem_free(stmt.sql.regist.ipaddr);
		mem_free(stmt.sql.regist.port);
		mem_free(stmt.sql.regist.service);
		stmt.type = 0;
		break;
		
	    case SQL_TYPE_UNREGISTER:
            case SQL_TYPE_UNSUBSCRIBE:
		mem_free(stmt.sql.unregist.id);
		stmt.type = 0;
		break;
		...
		if (stmt.sql.select.nfilters > 0) {
		    for (i = 0; i < stmt.sql.select.nfilters; i++) {
				...
				if (stmt.sql.select.filters[i]->IS_STR &&
					...
					mem_free(stmt.sql.select.filters[i]->value.stringv);	
				}
		...
		if (stmt.sql.select.groupby_ncols > 0) {
		    for (i = 0; i < stmt.sql.select.groupby_ncols; i++)
				mem_free(stmt.sql.select.groupby_cols[i]);
		    mem_free(stmt.sql.select.groupby_cols);
		}
		...
		mem_free(stmt.sql.update.tablename);
		if (stmt.sql.update.nfilters > 0) {
		    for (i = 0; i < stmt.sql.update.nfilters; i++) {
				mem_free(stmt.sql.update.filters[i]->varname);
		        mem_free(stmt.sql.update.filters[i]);
		    }
		    mem_free(stmt.sql.update.filters);
		}
		...
		if (stmt.sql.update.npairs > 0) {
		    for (i = 0; i < stmt.sql.update.npairs; i++) {
				mem_free(stmt.sql.update.pairs[i]->varname);
				if (stmt.sql.update.pairs[i]->IS_STR &&
					stmt.sql.update.pairs[i]->value.stringv) {
					mem_free(stmt.sql.update.pairs[i]->value.stringv);	
				}
		        mem_free(stmt.sql.update.pairs[i]);
		    }
		    mem_free(stmt.sql.update.pairs);
		}
...
void sql_print() {
	...
	switch(stmt.type) {
		...
		case SQL_TYPE_UNSUBSCRIBE:
		printf("Unsubscribed subscription, id = %s\n", stmt.sql.unregist.id);
		break;
