...
#define MAX_SUB_ID 0x7ffffff
static unsigned long gen_sub_id(void) {
...
unsigned long table_add_subscription(Table *tn, char *name, sqlsubscribe *subscribe, RpcConnection rpc) {
	Subscription *sub;
	unsigned long id;
	...
	sub->tablename = str_dupl(name);
	sub->rpc = rpc;
	id = gen_sub_id();
	ps_create(id, sub);
	
	list_append(tn->sublist, (void *)id);
	return id;
...
int table_persistent(Table *tn) { return (tn->tabletype); }
int table_key(Table *tn) { return (tn->primary_column); }
