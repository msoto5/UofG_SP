...
static int compts(int op, tstamp_t ts, tstamp_t then) {
    ...
    switch(op) {
        case LESS:	ans = (ts < then); break;
        case LESSEQ:	ans = (ts <= then); break;
        case GREATER:	ans = (ts > then); break;
        case GREATEREQ:	ans = (ts >= then); break;
        default:	ans = 0; break;
    }
    return ans;
}
...
static int compare(int op, void *cVal, int *cType, union filterval *filVal) {
	...
	switch(op) {
		case SQL_FILTER_EQUAL:
		return (strcmp(val, filVal->stringv) == 0); break;
		case SQL_FILTER_CONTAINS:
		return (strstr(val, filVal->stringv) != NULL); break;
...
void nodecrawler_update_cols(Nodecrawler *nc, Table *tn, sqlupdate *update) {
	...
	while (nodecrawler_has_more(nc)) {
		...
		for (i = 0; i < tn->ncols; i++) {
			...
			if (!value) 
				list_append(lcols, (void *)str_dupl(colVal));
			else
				list_append(lcols, (void *)(value));
		}
		ncols = list_len(lcols);
		if (ncols != tn->ncols) {
		...
		colvals = (char **) list_to_array(lcols);
		list_free(lcols);
		...
		/* if (value)
			mem_free(value); `value' is free'd below */
