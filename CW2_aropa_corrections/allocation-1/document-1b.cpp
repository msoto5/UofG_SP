...
//threadsafe unordered map
struct safeMap{
  private:
  std::unordered_map<std::string, std::list<std::string>> safeMap; 
  std::mutex mapMutex;

  public:

    int search_map(std::string key){
      //look in map, if not found, returns -1
      ...
      return 0;
        ...
        // ... insert mapping 
    int add_to_map(std::string key, std::list<std::string> value){
      std::unique_lock<std::mutex>lock(mapMutex);
      if (safeMap.count(key)){
        return 0;
      }
      else{
        safeMap.insert( { key, value } ); 
        return 1;
      }

    }
    //get address at specified index of map
    auto index_map(std::string key){
      std::unique_lock<std::mutex>lock(mapMutex);
      return &safeMap[key];
    }
};
...
struct safeQ {
  private:
  std::list<std::string> safeQ; std::mutex qMutex;
  public: 
  //add to queue 
  void appendQ(std::string name){
    std::unique_lock<std::mutex>lock(qMutex);
    safeQ.push_back( name );
  ...
  std::string see_front(){
    std::unique_lock<std::mutex>lock(qMutex);
    std::string front= safeQ.front();
    safeQ.pop_front();
    return front;
  } 
...
static void process(const char *file,   std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    //if already in table, skip, otherwise add 
    if (safeTable.search_map(name)==-1){
    // // ... insert mapping from file name to empty list in table ...
        safeTable.add_to_map(name, {});
    //... append file name to workQ
        safeWorkQ.appendQ(name);
    }
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
   ...
   std::list<std::string> *ll =safeTable.index_map(name);
...
//function with new threadsafe structs which calls process
void safeFunc(){
    while ( safeWorkQ.checkSize() > 0 ) {
      
      std::string filename = safeWorkQ.see_front();
      if (safeTable.search_map(filename)==-1){
        fprintf(stderr, "Mismatch between table and workQ\n");
      } 
      std::list<std::string> *ll = safeTable.index_map(filename); 
      process(filename.c_str(), ll);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  for (i = start; i < argc; i++) {
    ...
    safeTable.add_to_map(obj,{argv[i]});   
    // // 3b. insert mapping from file.ext to empty list
    safeTable.add_to_map(argv[i],{ });   
    // 3c. append file.ext on workQ
    safeWorkQ.appendQ(argv[i] );
  }

  
  std::vector<std::thread> v;
  //make vector of threads do the threadsafe function
  for (int i=0; i<worker_threads; i++){
      v.emplace_back([](){safeFunc();});
  }
