 ...
 */
			if (!TDS(fc) && !FDS(fc)){
				if (verbose) printf( "Ad hoc mode. Not supported yet.\n" );
				return;
			} else
			if (!TDS(fc) &&  FDS(fc)){
				// AP to STA.
			...
			if( TDS(fc) &&  FDS(fc)) {
				if (verbose) printf( "Not defined yet in standard.\n" );
				return;
			}
			break;
		case TYPE_RESV:
			if (verbose) printf( "Type is reserved. Not supported yet.\n" );
			return;
...
static void *handler(void *args) {
	...
	int size;
	unsigned long elements; /* Elements in hash table. */
	...
	unsigned long links; /* Links written in buffer so far. */
	...
	while (1) {
		...
		buckets = lt_swap(&size, &elements);
		if (elements <= 0) /* Nothing to do. */
			continue;
		if (verbose > 0)
			printf("%ld elements accumulated\n", elements);
		...
		for (i = 0; i < size; i++) {
			...
			while (p != NULL) {
				...
				"insert into Links values (\"%02x:%02x:%02x:%02x:%02x:%02x\", \
'%f', '%ld', '%ld', '%ld')\n",
				(int) ((f->mac >>40) & 0xff),
				(int) ((f->mac >>32) & 0xff),
				(int) ((f->mac >>24) & 0xff),
				...
				(int) ((f->mac >> 8) & 0xff),
				(int) ((f->mac     ) & 0xff),
				p->avg_rss, p->retries, p->packets, p->nobytes);
