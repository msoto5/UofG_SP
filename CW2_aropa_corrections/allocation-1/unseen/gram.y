...
sqlStmt:  EXEC WORD {
        ...
        | SUBSCRIBE WORD IPADDR NUMBER WORD {
            debugvf("Subscribe statement: query: %s;", (char *)$2);
            debugvf(" ip:port:service: %s:%s:%s\n", 
...
selectStmt:
          SELECT all FROM tableList {
            orderby = NULL;
          }
        | SELECT all FROM tableList WHERE filterList {
            orderby = NULL;
          }
        ...
        | SELECT colList FROM tableList ORDER BY orderList {
            countstar = 0;
          }
...
filter:
        ...
        | WORD CONTAINS constant {
            ...
            if (!flist)
              flist = list_new();
            tmpfilter = sqlstmt_new_filter(CONTAINS, (char*)$1,
                                           tmpvaltype, tmpvalstr);
            list_append(flist, (void *)tmpfilter);
            mem_free(tmpvalstr);
          }
        | WORD NOTCONTAINS constant {
            ...
            if (!flist)
              flist = list_new();
            tmpfilter = sqlstmt_new_filter(NOTCONTAINS, (char*)$1,
                                           tmpvaltype, tmpvalstr);
            list_append(flist, (void *)tmpfilter);
            mem_free(tmpvalstr);
          }
