    ...
    for (;;) {
        if (oldestT < nextT) {		/* oldestT behind nextT */
            if ((nextT + alloc_len) >= lastPtr) {
				...
				passes++;
				/* Log result */
				fprintf(stderr, "At %llu buffer wrapped %ld times\n", timestamp_now(), passes);
	    	} else
                break; /* OK */
		} else {		/* oldestT behind nextT */
            if ((nextT + alloc_len >= oldestT)) {
...
int heap_insert_tuple(int ncols, char *vals[], Table *tb, Node *node) {
	...
	buf = mem_alloc(alloc_len);
	if (!buf) {
		fprintf(stderr, "Out of memory\n");
		return 0;
	};
	if (! (n = node)) {
		n = mem_alloc(sizeof(Node));
		if (!n) {
			fprintf(stderr, "Out of memory\n");
			mem_free(buf);
			return 0;
...
void mb_dump() {
	...
	fprintf(stderr, 
	"%llu: bytes %ld nodes %ld (bytes %ld) bytes/tuple %.2f free %ld pass %ld\n",
	timestamp_now(), 
	nbytes, 
	nnodes, 
	bnodes, 
